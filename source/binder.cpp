...
// check if declaration is already in stack with level at least as 'level' or
...
llvm::raw_ostream &operator<<(llvm::raw_ostream &os, Binder const &b) {
  ...
  string path = decl->getQualifiedNameAsString().substr(
      0, qualified_name.size() - name.size());
