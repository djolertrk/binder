...
// structure to hold include set information and set of NamedDecl objects that
// was already queried for includes
class IncludeSet {
...
public:
  ...
  // check if declaration is already in stack with level at lease as 'level' or
  // lower and add it if it is not - return true if declaration was added
...
/// Bindings Generator - represent object that can generate binding info for
/// function, class, enum or data variable
class Binder {
public:
  ...
  /// check if user supplied config requested binding for the given declaration
  ...
  virtual std::vector<clang::CXXRecordDecl const *> dependencies() const {
    return std::vector<clang::CXXRecordDecl const *>();
  }
  ...
  explicit operator std::string() const {
    return id(); /*named_decl()->getQualifiedNameAsString();*/
  }
