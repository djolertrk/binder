...
// generate class template specialization for ClassTemplateSpecializationDecl or
...
string template_specialization(clang::CXXRecordDecl const *C) {
  ...
  if (auto t = dyn_cast<ClassTemplateSpecializationDecl>(C)) {
    ...
    for (uint i = 0; i < t->getTemplateArgs().size(); ++i) {
      // if( t->getTemplateArgs()[i].isInstantiationDependent() ) break;  //
      // avoid explicitly specifying SFINAE related arguments

      // outs() << " template argument: " <<
      // template_argument_to_string(t->getTemplateArgs()[i]) << "\n"; templ +=
      // template_argument_to_string(t->getTemplateArgs()[i]) + ",";
      std::string template_arg =
          template_argument_to_string(t->getTemplateArgs()[i]);
      if ((template_arg[0] == '<') and
          (template_arg[template_arg.size() - 1] == '>'))
        template_arg = template_arg.substr(1, template_arg.size() - 2);
      if (template_arg.size() > 0)
        templ += template_arg + ",";

      // if( t->getTemplateArgs()[i].ArgKind() ==
      // TemplateArgument::ArgKind::Integral ) outs() << " template arg:" <<
      // t->getTemplateArgs()[i].<< "\n"; outs() << expresion_to_string(
      // t->getTemplateArgs()[i].getAsExpr() ) << "\n";
    }
    templ.back() = '>';
  }
...
// generate qualified class name that could be used in bindings code indcluding
// template specialization if any
string class_qualified_name(CXXRecordDecl const *C) {
  return standard_name(C->getQualifiedNameAsString() +
                       template_specialization(C));
}

// generate vector<QualType> with all types that class uses if it tempalated
vector<QualType>
get_type_dependencies(CXXRecordDecl const *C /*, bool include_members*/) {
  vector<QualType> r;

  if (auto t = dyn_cast<ClassTemplateSpecializationDecl>(C)) {
    for (uint i = 0; i < t->getTemplateArgs().size(); ++i) {
      if (t->getTemplateArgs()[i].getKind() == TemplateArgument::Type) {
        r.push_back(t->getTemplateArgs()[i]
...
// Return true if class have direct or inderect std::enable_shared_from_this as
// base class
bool is_inherited_from_enable_shared_from_this(CXXRecordDecl const *C) {
  if (C->getQualifiedNameAsString() == "std::enable_shared_from_this")
    return true;
  if (C->isCompleteDefinition()) {
    for (auto b = C->bases_begin(); b != C->bases_end(); ++b) {
      if (auto r = dyn_cast<RecordType>(
              b->getType().getCanonicalType().getTypePtr())) {
        CXXRecordDecl *rd = cast<CXXRecordDecl>(r->getDecl());
        if (rd /*and rd->isCompleteDefinition()*/ and
            is_inherited_from_enable_shared_from_this(rd))
          return true;
      }
    }
  }
...
bool is_field_assignable(FieldDecl const *f) {
  if (RecordType const *r = dyn_cast<RecordType>(f->getType())) {
    if (CXXRecordDecl *C = cast<CXXRecordDecl>(
            r->getDecl())) { // checking if this type has deleted operator=
      for (auto m = C->method_begin(); m != C->method_end(); ++m) {
        // if( m->getAccess() == AS_public  and  m->isCopyAssignmentOperator()
        // /*and  !m->doesThisDeclarationHaveABody()*/ and  m->isDeleted() )
        // return false;
        if (m->isCopyAssignmentOperator() and
            (m->getAccess() != AS_public or m->isDeleted()))
          return false;
      }
    }
  }
...
bool is_bindable(FieldDecl *f) {
  if (f->getType()->isAnyPointerType() or f->getType()->isReferenceType() or
...
string bind_data_member(FieldDecl const *d,
  ...
  for (const auto &anonymous : anonymous_types) {
    if (ends_with(class_qualified_name, anonymous))
      class_qualified_name.resize(class_qualified_name.size() -
                                  anonymous.size());
  }

  if (d->getType().isConstQualified() or !is_field_assignable(d))
    return ".def_readonly(\"{}\", &{}::{})"_format(
        d->getNameAsString(), class_qualified_name, d->getNameAsString());
  else
    return ".def_readwrite(\"{}\", &{}::{})"_format(
        d->getNameAsString(), class_qualified_name, d->getNameAsString());
}

// Check if given method is const-overload (and return-const-overload) of some
...
bool is_const_overload(CXXMethodDecl *mc) {
  if (mc->isConst()) {
    ...
    if (name.size() > _const.size()) {
      ...
      for (auto m = C->method_begin(); m != C->method_end(); ++m) {
        if (m->getAccess() == mc->getAccess() and !m->isConst() and
            function_qualified_name(*m, true) == name)
          return true;
      }
    }
  }

  // outs() << "is_const_overload: " << function_qualified_name(mc, false) << "
  // -> false\n\n";

  return false;
}

// check if given CXXRecordDecl (which known to be template specialization of
// std::function) is bindable
  ...
  if (auto t = dyn_cast<ClassTemplateSpecializationDecl>(C)) {
    ...
    for (uint i = 0; i < t->getTemplateArgs().size(); ++i) {
      ...
      if (t->getTemplateArgs()[i].getKind() == TemplateArgument::Declaration) {
        ...
        if (FunctionProtoType const *ft =
                dyn_cast<FunctionProtoType>(qt.getTypePtr())) {
          if (not is_bindable(ft->getReturnType()))
            return false;
          for (uint i = 0; i < ft->getNumParams(); ++i) {
            if (not is_bindable(ft->getParamType(i)))
              return false;
          }
        }
      }
...
bool is_bindable_raw(clang::CXXRecordDecl const *C) {
  ...
  // outs() << "is_bindable(CXXRecordDecl): " << C->getQualifiedNameAsString()
  // << template_specialization(C)
  // 	   << " C->hasDefinition():" << C->hasDefinition()
  // 	   << " C->isCompleteDefinition():" << C->isCompleteDefinition()
  // 	   // << " C->isThisDeclarationADefinition():" <<
  // C->isThisDeclarationADefinition()
  ...
  // enabling binding of anonymous classes and handle it as special case in
  // `bind_nested_classes` if( qualified_name == "(anonymous)" ) return false;
  // if( qualified_name.rfind(')') != std::string::npos ) return false; // check
  // for anonymous structs and types in anonymous namespaces

  // disabling bindings for anonymous namespace's
  // if( qualified_name.rfind("(anonymous namespace)") != std::string::npos )
  // return false;
  ...
  bool anonymous_name =
      qualified_name.rfind(')') !=
      std::string::npos; // check if type name is "(anonymous)"
  if (anonymous_name and C->hasNameForLinkage())
    return false;
  if (anonymous_name and !C->hasNameForLinkage() and
      !C->isAnonymousStructOrUnion())
    return false;

  // if( C->isAnonymousStructOrUnion() and C->hasNameForLinkage() ) return
  // false;

  // outs() << qualified_name << ": anonymous_name:" << anonymous_name << "
  // isAnonymousStructOrUnion: " << C->isAnonymousStructOrUnion() << "
  // hasNameForLinkage:" << C->hasNameForLinkage() << "\n";
  ...
  if (!C->isCompleteDefinition()) {
    if (auto ts = dyn_cast<ClassTemplateSpecializationDecl>(C)) {
      if (qualified_name == "std::function") {
        ...
        // if( ts->getPointOfInstantiation() /* SourceLocation */.isInvalid()
        // and not is_python_builtin(C) ) {
        //  outs() << "is_bindable( " << qualified_name << " " <<
        //  class_qualified_name(C) << " ): no point of instantiation  found,
        //  skipping...\n";
        return true;
      }
    } else
...
bool is_binding_requested(clang::CXXRecordDecl const *C, Config const &config) {
  if (config.is_class_binding_requested(class_qualified_name(C)) or
      config.is_class_binding_requested(
          standard_name(C->getQualifiedNameAsString())))
    return true;

  static bool bind_class_template_specialization =
      O_bind_class_template_specialization;

  if ((not bind_class_template_specialization) and
      dyn_cast<ClassTemplateSpecializationDecl>(C))
    return false;

  bool bind =
      config.is_namespace_binding_requested(namespace_from_named_decl(C));
  for (auto &t : get_type_dependencies(C))
    bind &= !is_skipping_requested(t, config);
  return bind;
}

// check if user requested skipping for the given declaration
bool is_skipping_requested(clang::CXXRecordDecl const *C,
                           Config const &config) {
  ...
  if (config.is_class_skipping_requested(
          qualified_name_with_template_specialization))
    return true;
  if (config.is_class_binding_requested(
          qualified_name_with_template_specialization))
    return false;

  if (config.is_class_skipping_requested(qualified_name))
    return true;
  if (config.is_class_binding_requested(qualified_name))
    return false;
...
void add_relevant_includes(clang::CXXRecordDecl const *C, IncludeSet &includes,
  ...
  if (auto t = dyn_cast<ClassTemplateSpecializationDecl>(C)) {
    ...
    for (uint i = 0; i < t->getTemplateArgs().size(); ++i) {
      ...
      if (t->getTemplateArgs()[i].getKind() == TemplateArgument::Template) {
        add_relevant_include_for_decl(t->getTemplateArgs()[i]
                                          .getAsTemplate()
                                          .getAsTemplateDecl()
                                          ->getTemplatedDecl(),
                                      includes);
  ...
  // member function templates: we have to treat them separatly because
  // instantiation types might not be included in class header
  for (auto d = C->decls_begin(); d != C->decls_end(); ++d) {
    if (FunctionTemplateDecl *ft = dyn_cast<FunctionTemplateDecl>(*d)) {
      for (auto s = ft->spec_begin(); s != ft->spec_end(); ++s) {
        if (CXXMethodDecl *m = dyn_cast<CXXMethodDecl>(*s)) {
          if (m->getAccess() == AS_public and
              ...
              // const overload functions might be not bound but still
      ...
      // 	outs() << "function template argument: " <<
      // template_argument_to_string( *a ) << " type:" << a->getKind() << "\n";
      // 	if( a->isDependent() ) {
      // 		outs() << "isDependent!" << "\n";
      // 	}
  ...
  // outs() << "isCompleteDefinition:" << C->isCompleteDefinition() << " id: "
...
inline void add_includes_to_set(std::vector<std::string> const &from,
                                IncludeSet &to) {
  ...
  for (auto b = C->bases_begin(); b != C->bases_end(); ++b) {
    if (auto rt = dyn_cast<RecordType>(
      ...
      if (CXXRecordDecl *R = cast<CXXRecordDecl>(rt->getDecl())) {
        ...
        if (!R->hasDefaultConstructor())
          return false;

        for (auto t = R->ctor_begin(); t != R->ctor_end(); ++t) {
          if (t->isDefaultConstructor()) {
            if (t->getAccess() == AS_private /*or  !t->isUserProvided()*/ or
                t->isDeleted())
              return false;
          }
        }

        if (!base_default_default_constructor_available(R))
          return false;
      }
    }
  }
...
/// this is function prototype and for classes forward declaration.
...
string generate_opaque_declaration_if_needed(
    string const &qualified_name,
    string const &qualified_name_without_template) {
  // pybind11 container lists
  // https://pybind11.readthedocs.io/en/stable/advanced/cast/stl.html
  static vector<string> stl_containers{
      "std::vector",        "std::deque",    "std::list",
      "std::array",         "std::valarray", "std::set",
      "std::unordered_set", "std::map",      "std::unordered_map"};

  if (begins_with(qualified_name_without_template, "std::")) {
    auto it = std::find(stl_containers.begin(), stl_containers.end(),
...
string binding_public_data_members(CXXRecordDecl const *C) {
  ...
  // binding protected data member that was made public in child class by
  // 'using' declaration
  for (auto d = C->decls_begin(); d != C->decls_end(); ++d) {
        ...
        for (auto s = u->shadow_begin(); s != u->shadow_end(); ++s) {
          if (UsingShadowDecl *us = dyn_cast<UsingShadowDecl>(*s)) {
            if (FieldDecl *f = dyn_cast<FieldDecl>(us->getTargetDecl())) {
              ...
              if (config.is_field_skipping_requested(
  ...
  if (C->isAbstract()) {
    for (auto m = C->method_begin(); m != C->method_end(); ++m) {
      if (m->isPure() and is_const_overload(*m))
        return false; // it is not clear how to deal with this case since we
                      // can't overrdie const versions in Python, - so disabling
                      // for now
    }
  }
...
// call_back_function_body_template is almost like PYBIND11_OVERLOAD_INT but
// specify pybind11::return_value_policy::reference
...
// 			} \
// 			else return
// pybind11::detail::cast_safe<ret_type>(std::move(o)); \
// 		} \
// 	}
  ...
  for (auto m = C->method_begin(); m != C->method_end(); ++m) {
    if ((m->getAccess() != AS_private) and is_bindable(*m) and
        ...
        and !is_const_overload(
                *m) // was ( !is_const_overload(*m) or m->isPure() ) but now we
                    // ignoring this case since it is not possible to overload
                    // both const and non-const variants in Python anyway
    ) {
      ...
      tuple<string, string, string> args =
          function_arguments_for_py_overload(*m);

      string key = /*return_type + ' ' +*/ m->getNameAsString() + '(' +
                   std::get<0>(args) + (m->isConst() ? ") const" : ")");
      ...
      if (!binded.count(key)) {
        ...
        // m->hasAttr<OverrideAttr>, attribute list is automatically generated,
        // see <release>/tools/clang/include/clang/Basic/AttrList.inc for
        // attribute list
        if (m->hasAttr<FinalAttr>())
          continue; // we can not test this condition in a big if above because
                    // we need 'final' function to be marked as 'binded' so they
                    // will be binded by some of the base classes
        ...
        if (FunctionProtoType const *fpt =
                dyn_cast<FunctionProtoType>(m->getType().getTypePtr())) {
          if (fpt->getExceptionSpecType() &
              clang::ExceptionSpecificationType::EST_BasicNoexcept)
            exception_specification = "noexcept ";
          if (fpt->getExceptionSpecType() &
              (clang::ExceptionSpecificationType::EST_DynamicNone |
               clang::ExceptionSpecificationType::EST_Dynamic |
               clang::ExceptionSpecificationType::EST_MSAny))
            exception_specification = "throw() ";
        }

        c += "\t{} {}({}){} {}override {{"_format(
            return_type, m->getNameAsString(), std::get<0>(args),
            m->isConst() ? " const" : "", exception_specification);

        string member_function_name = namespace_from_named_decl(C);
        if (member_function_name.length() > 0)
          member_function_name += "::";
        member_function_name +=
            C->getNameAsString() + "::" + m->getNameAsString();
        string custom_function_info =
            Config::get().is_custom_trampoline_function_requested(
                member_function_name);
        if (custom_function_info == "") {
          c += indent(fmt::format(call_back_function_body_template, class_name,
                                  /*class_qualified_name(C), */ python_name,
                                  std::get<1>(args), return_type),
                      "\t\t");
          if (m->isPure())
            c +=
                "\t\tpybind11::pybind11_fail(\"Tried to call pure virtual function \\\"{}::{}\\\"\");\n"_format(
                    C->getNameAsString(), python_name);
          else
            c += "\t\treturn {}::{}({});\n"_format(
                C->getNameAsString(), m->getNameAsString(), std::get<1>(args));
        } else {
        ...
        // c += string(m->isPure() ? "PYBIND11_OVERLOAD_PURE_NAME" :
        // "PYBIND11_OVERLOAD_NAME") + '('; c += return_type + ", " +
        // base_type_alias + ", \"" + python_function_name(*m) + "\", " +
        // C->getNameAsString() + "::" + m->getNameAsString(); c += (
        // args.second.size() ? ", " + args.second : ""); c += "); }\n";

        // Note: does not work when argument is std::shared_ptr passed
        // by-reference... string python_name = python_function_name(*m); c +=
        // "PYBIND11_OVERLOAD_INT({}, \"{}\"{}); "_format(return_type,
        // python_name, std::get<2>(args).size() ? ", " + std::get<2>(args) :
        // ""); if( m->isPure() ) c+= "pybind11::pybind11_fail(\"Tried to call
        // pure virtual function
        // \\\"{}::{}\\\"\");"_format(C->getNameAsString(), python_name); else
        // c+= "return {}::{}({});"_format(C->getNameAsString(),
        // m->getNameAsString(), std::get<1>(args)); c += " }\n";
...
string binding_template_bases(CXXRecordDecl const *C, bool callback_structure,
                              bool callback_structure_constructible,
                              Context &context) {
  string c;

  if (dyn_cast<ClassTemplateSpecializationDecl>(C) and
      C->getDefinition()) { // for template classes explicitly bind data members
                            // and member functions from public base classes
    for (auto b = C->bases_begin(); b != C->bases_end(); ++b) {
...
/// Create forward-binding for given class which consist of only class type
/// without any member, function or constructors
string bind_forward_declaration(CXXRecordDecl const *C, Context &context) {
  string const qualified_name{class_qualified_name(C)};
  string const module_variable_name =
      context.module_variable_name(namespace_from_named_decl(C));
  ...
  string c = "\t// Forward declaration for: " + qualified_name + " file:" +
             (include.size() ? include.substr(1, include.size() - 2) : "") +
             " line:" + line_number(C) + "\n";
...
/// check if any of the base classes is wrappable and if generate a string
...
string ClassBinder::maybe_base_classes(Context &context) {
  ...
  static std::vector<string> const skip_list = {
      "std::enable_shared_from_this", "std::string", "std::basic_string",
      "std::pair", "std::tuple"};
  ...
  for (auto b = C->bases_begin(); b != C->bases_end(); ++b) {
    if (b->getAccessSpecifier() == AS_public) {
      if (auto rt = dyn_cast<RecordType>(
              b->getType().getCanonicalType().getTypePtr())) {
        if (CXXRecordDecl *R = cast<CXXRecordDecl>(rt->getDecl())) {
          auto e = std::find(skip_list.begin(), skip_list.end(),
                             standard_name(R->getQualifiedNameAsString()));

          if (e == skip_list.end() and is_bindable(R) and
              !is_skipping_requested(R, Config::get())) {
...
void ClassBinder::bind_with(string const &binder, Context &context) {
  ...
  c += '\t' + binder + standard_name(template_specialization(C)) + '(' +
       module_variable_name;

  if (auto t = dyn_cast<ClassTemplateSpecializationDecl>(C)) {
    for (uint i = 0; i < t->getTemplateArgs().size(); ++i) {

      string templ = mangle_type_name(
          standard_name(template_argument_to_string(t->getTemplateArgs()[i])),
          true);
      fix_boolean_types(templ);
      c += ", \"" + templ + '"';
    }
...
// char const * constructor_template = "\tcl.def(\"__init__\", []({2} *self_{0})
// {{ new (self_) {2}({1}); }}, \"doc\");"; char const * constructor_if_template
// = "\tcl.def(\"__init__\", [cl_type](pybind11::handle self_{0}) {{ if
// (self_.get_type() == cl_type) new (self_.cast<{2} *>()) {2}({1}); else new
// (self_.cast<{3}
// *>()) {3}({1}); }}, \"doc\");";

char const *constructor_template =
    "\tcl.def( pybind11::init( []({0}){{ return new {2}({1}); }} ), \"doc\");";
char const *constructor_template_with_py_arg =
    "\tcl.def( pybind11::init( []({0}){{ return new {2}({1}); }} ), \"doc\" "
    "{3});";
char const *constructor_with_trampoline_template =
    "\tcl.def( pybind11::init( []({0}){{ return new {2}({1}); }}, []({0}){{ "
    "return new {3}({1}); }} ), \"doc\");";
// char const * constructor_lambda_template = "\tcl.def(pybind11::init(
// []({0}){{ return new {2}({1}); }}, []({0}){{ return new {3}({1}); }} ),
// \"doc\");";

// Generate binding for given function: .def("foo", (std::string
// (aaaa::A::*)(int) ) &aaaa::A::foo, "doc") constructor_types is pair<Base,
// Alias> - if one of these is absent empty string is expected string
// bind_constructor(CXXConstructorDecl const *T, pair<string, string> const
// &constructor_types, uint args_to_bind, bool request_bindings_f, Context
// &context)
string bind_constructor(ConstructorBindingInfo const &CBI, uint args_to_bind,
  ...
  if (args_to_bind == CBI.T->getNumParams() and not CBI.T->isVariadic()) {
    ...
    pair<string, string> args =
        function_arguments_for_lambda(CBI.T, args_to_bind);
    ...
    for (uint i = 0; i < CBI.T->getNumParams() and i < args_to_bind; ++i) {
      args_helper += ", pybind11::arg(\"{}\")"_format(
          string(CBI.T->getParamDecl(i)->getName()));
    }

    // if( CBI.T->isVariadic() ) c = fmt::format(constructor_lambda_template,
    // params, args.second, constructor_types.first, constructor_types.second);
    // else if( constructor_types.first.size()  and
    // constructor_types.second.size() ) c =
    // fmt::format(constructor_lambda_template, params, args.second,
    // constructor_types.first, constructor_types.second); else if(
    // constructor_types.first.size() ) c = fmt::format(constructor_template,
    // params, args.second, constructor_types.first); else c =
    // fmt::format(constructor_template, params, args.second,
    // constructor_types.second);

    if (CBI.C->isAbstract())
      c = fmt::format(constructor_template, params, args.second,
                      CBI.trampoline_qualified_name);
    else if (CBI.trampoline)
      c = fmt::format(constructor_with_trampoline_template, params, args.second,
                      CBI.class_qualified_name, CBI.trampoline_qualified_name);
    else
      c = fmt::format(constructor_template_with_py_arg, params, args.second,
                      CBI.class_qualified_name, args_helper);
  }
...
string bind_default_constructor(
    ConstructorBindingInfo const
        &CBI) // CXXRecordDecl const *, string const & binding_qualified_name)
{
...
/// Generate copy constructor in most cases this will be just:
/// "\tcl.def(pybind11::init<{} const &>());\n"_format(binding_qualified_name);
/// but for POD structs with zero data mambers this will be a lambda function.
/// This is done as a workaround for Pybind11 2,2+ bug
string bind_copy_constructor(
    ConstructorBindingInfo const &CBI) // CXXConstructorDecl const *T, string
                                       // const & binding_qualified_name)
{
  // CXXRecordDecl const *C = T->getParent();

  // C->dump();
  // outs() << "isCLike=" << C->isCLike() << " isTrivial=" << T->isTrivial() <<
  // " isEmpty=" << C->isEmpty() << " isPOD=" << C->isPOD() << " isAggregate="
  // << C->isAggregate() << "\n";

  // bool use_class_copy_constructor = (not C->isCLike() ) and ( (not
  // T->isTrivial())  or  (not C->isEmpty()) ); if(use_class_copy_constructor)
  // return "\tcl.def(pybind11::init<{} const
  // &>());\n"_format(binding_qualified_name); else return "\tcl.def(
  // pybind11::init( []({0} const &o){{ return new {0}(o); }} )
  // );\n"_format(binding_qualified_name);

  // bool generate_init_with_lambda = CBI.C->isAggregate()  or
  // CBI.callback_structure_constructible;  //C->isCLike() or (T->isTrivial()
  // and  C->isEmpty()); if(generate_init_with_lambda) { 	if(
  ...
  // &>());\n"_format( CBI.binding_qualified_name ); 	else return
  // "\tcl.def(pybind11::init<{} const &>());\n"_format(
  // CBI.class_qualified_name );
  // }
  ...
  if (CBI.trampoline) {
    if (CBI.C->isAbstract())
      return "\tcl.def(pybind11::init<{}{} &>());\n"_format(
          CBI.trampoline_qualified_name, const_bit);
    else {
      // not yet supported by Pybind11? return "\tcl.def( pybind11::init( []({0}
      // const &o){{ return new {0}(o); }}, []({1} const &o){{ return new
      // {1}(o); }} )
      // );\n"_format(CBI.class_qualified_name, CBI.binding_qualified_name);
      return "\tcl.def( pybind11::init( []({0}{1} &o){{ return new {0}(o); }} ) );\n"_format(
                 CBI.trampoline_qualified_name, const_bit) +
             (CBI.T->getAccess() == AS_public
                  ? "\tcl.def( pybind11::init( []({0}{1} &o){{ return new {0}(o); }} ) );\n"_format(
                        CBI.class_qualified_name, const_bit)
                  : "");
    }
  } else
    return "\tcl.def( pybind11::init( []({0}{1} &o){{ return new {0}(o); }} ) );\n"_format(
...
// Generate binding for given constructor. If constructor have default arguments
// generate set of bindings by creating separate bindings for each argument with
// default.
...
/// generate (if any) bindings for Python __str__ by using appropriate global
/// operator<<
std::string ClassBinder::bind_repr(Context &context, Config const &config) {
  string c;
  string qualified_name = class_qualified_name(C);
  if (config.is_function_skipping_requested(qualified_name + "::__str__") or
      config.is_function_skipping_requested(
          standard_name(C->getQualifiedNameAsString() + "::__str__")))
    return c;

  if (FunctionDecl const *F = context.global_insertion_operator(C)) {
    // outs() << "Found insertion operator for: " << class_qualified_name(C) <<
    // "\n"; outs() << "insertion operator: " << F->getNameInfo().getAsString()
    // << " qn: " << F->getQualifiedNameAsString() << " dn:" << F->getDeclName()
    // << "\n";
    ...
    if (ns.size())
      maybe_using_decl = " using namespace {};"_format(ns);

    // c += "\n\tcl.def(\"__str__\", []({} const &o) -> std::string {{
    // std::ostringstream s; {}(s, o); return s.str(); }}
    // );\n"_format(qualified_name, F->getQualifiedNameAsString());
    c +=
        "\n\tcl.def(\"__str__\", []({} const &o) -> std::string {{ std::ostringstream s;{} s << o; return s.str(); }} );\n"_format(
            qualified_name, maybe_using_decl);
...
void ClassBinder::for_public_nested_classes(
    std::function<void(clang::CXXRecordDecl const *)> const &f) const {
  for (auto d = C->decls_begin(); d != C->decls_end(); ++d) {
...
string ClassBinder::bind_nested_classes(Context &context) {
  ...
  for_public_nested_classes([&c, &context, this](CXXRecordDecl const *innerC) {
    if (!innerC->isFirstDecl())
      return; // avoid multiple bindings: try to bind innerC only at first
              // declaration
...
void ClassBinder::bind(Context &context) {
  ...
  // prefix_code_ += generate_opaque_declaration_if_needed(qualified_name,
  // qualified_name_without_template);
  ...
  // if( named_class and (qualified_name.rfind(')') != std::string::npos) )
  // named_class = false; // check for anonymous structs and types in anonymous
  // namespaces

  string const module_variable_name =
      C->isCXXClassMember() and named_class
          ? "enclosing_class"
          : context.module_variable_name(namespace_from_named_decl(C));
  // string const decl_namespace = namespace_from_named_decl(C);
  ...
  string maybe_trampoline =
      callback_structure_constructible ? ", " + binding_qualified_name : "";
  ...
  if (Config::get().is_buffer_protocol_requested(
          qualified_name_without_template))
  ...
  if (named_class) {
    ...
    if (Config::get().is_smart_holder_requested(
            qualified_name_without_template)) {
      c += '\t' +
           R"(PYBIND11_TYPE_CASTER_BASE_HOLDER({} {}))"_format(
  ...
  if (((C->getDefinition() and !C->isAbstract()) or
    ...
    // Generate bindings for constructors inherited from base classes though
    // `using` declaration
    for (auto d = C->decls_begin(); d != C->decls_end(); ++d) {
      if (UsingDecl *u = dyn_cast<UsingDecl>(*d)) {
        if (u->getAccess() == AS_public) {
              ...
              if (CXXConstructorDecl *t =
                ...
                if (is_bindable(t)) {
                  ...
                  if( t->isCopyConstructor() /*and  not copy_constructor_processed*/ ) {
                    ...
                    // constructors += bind_copy_constructor(t,
                    // binding_qualified_name); copy_constructor_processed =
                    // true;
                  } else if (t->isDefaultConstructor() and
                             t->getNumParams() == 0)
                    constructors += bind_default_constructor(
                        CBI); // workaround for Pybind11-2.2 issues
                  else
    ...
    // c += "\t// hasDefaultConstructor={} needsImplicitDefaultConstructor={}
    // base_default_default_constructor_available={}\n"_format(C->hasDefaultConstructor(),
    // C->needsImplicitDefaultConstructor(),
    // base_default_default_constructor_available(C));
    //		C->isStruct() && C->hasDefaultConstructor()

    if (!default_constructor_processed and
        C->needsImplicitDefaultConstructor() and
        base_default_default_constructor_available(C) and
        !C->isStruct() // AADC Disable default constructor for struct
                       // AADC to implement this properly we need to if
                       // AADC there are any const data members.
                       // ( C->ctor_begin() == C->ctor_end() and
                       // is_default_default_constructor_available(C)  and
        // !default_constructor_processed) or (C->hasDefaultConstructor()  and
        // !default_constructor_processed )

        // if( C->hasDefaultConstructor()  or ( !default_constructor_processed
        // and  is_default_default_constructor_available(C)) /*and
        // !C->needsImplicitDefaultConstructor() and
        // !C->hasNonTrivialDefaultConstructor()*/

        /*and  !C->needsImplicitDefaultConstructor() and
           !C->hasNonTrivialDefaultConstructor()*/
    ) { // No constructors defined, adding default constructor

      // c += "\tcl.def(pybind11::init<>());__\n";  // making sure that default
      // is appering first
      c += bind_default_constructor(ConstructorBindingInfo{
          C, nullptr, trampoline, qualified_name, trampoline_name,
          context}); // making sure that default is appering first
    }
    c += constructors;
  }
  ...
  std::map<string, string> const &external_add_on_binders =
      Config::get().add_on_binders();
