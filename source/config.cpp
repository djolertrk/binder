...
std::pair<string, string> split_in_two(string const &s,
  ...
  if (split == string::npos) {
    ...
    return std::make_pair(s.substr(0, split), s.substr(split + 1));
}
...
void Config::read(string const &file_name) {
  if (O_verbose)
    outs() << "Read config from file " << file_name << "\n";
  ...
  string const _default_static_lvalue_reference_return_value_policy_{
  ...
  string const _default_member_pointer_return_value_policy_{
      "default_member_pointer_return_value_policy"};
  string const _default_member_lvalue_reference_return_value_policy_{
  ...
  string const _trampoline_member_function_binder_{
      "trampoline_member_function_binder"};

  string const _default_call_guard_{"default_call_guard"};

  string const _prefix_for_static_member_functions_{
  ...
  while (std::getline(f, line)) {
    ...
    if (line[0] != '+' and line[0] != '-') {
      outs() << "Error: Invalid token at the begining of line in config file! "
                "Each line should begin with ether '+' or '-' or '#'! Line: "
             << line << "\n";
      assert(false);
      //	throw std::runtime_error("Invalid token at the begining of line
      //in config file! Each line should begin with ether '+' or '-' or '#'!
      //Line: " + line);
    }
    outs() << "line: " << line << "\n";
    size_t space = line.find(' ');
    if (space == string::npos) {
      outs() << "Error: Invalid line in config file! Each line must have token "
                "separated with space from object name. For example: "
                "'+function aaa::bb::my_function'. Line: "
             << line << "\n";
      assert(false);
      // throw std::runtime_error("Invalid line in config file! Each line must
      // have token separated with space from object name. For example:
      // '+function aaa::bb::my_function'. Line: " + line);
    }
    ...
    name_without_spaces.erase(std::remove(name_without_spaces.begin(),
    ...
    if (token == _namespace_) {
      ...
      if (bind)
        namespaces_to_bind.push_back(name_without_spaces);
      else
        namespaces_to_skip.push_back(name_without_spaces);
    } else if (token == _class_) {

      if (bind)
        classes_to_bind.push_back(name_without_spaces);
      else
        classes_to_skip.push_back(name_without_spaces);
    } else if (token == _enum_) {

      if (bind)
        enums_to_bind.push_back(name_without_spaces);
      else
        enums_to_skip.push_back(name_without_spaces);
    } else if (token == _python_builtin_) {

      if (bind)
        python_builtins.insert(name_without_spaces);
      else
        not_python_builtins.insert(name_without_spaces);
    } else if (token == _function_) {

      if (bind)
        functions_to_bind.push_back(name_without_spaces);
      else
        functions_to_skip.push_back(name_without_spaces);
      ...
      if (bind) {
        auto class_and_include = split_in_two(
            name, "Invalid line for include_for_class specification! Must be: "
                  "name_of_type + <space or tab> + include_path. Got: " +
                      line);
        class_includes_[class_and_include.first].push_back(
            class_and_include.second);
      } else {
        outs() << "Error: include_for_class must be '+' configuration. Line: "
               << line << "\n";
        assert(false);
        // throw std::runtime_error("include_for_class must be '+'
        // configuration.");
      }
    } else if (token == _include_for_namespace_) {

      if (bind) {
        auto namespace_and_include = split_in_two(
            name, "Invalid line for include_for_namespace specification! Must "
                  "be: name_of_type + <space or tab> + include_path. Got: " +
                      line);
        namespace_includes_[namespace_and_include.first].push_back(
            namespace_and_include.second);
      } else {
        outs()
            << "Error: include_for_namespace must be '+' configuration. Line: "
            << line << "\n";
        assert(false);
        // throw std::runtime_error("include_for_namespace must be '+'
        // configuration.");
      }
      ...
      if (bind) {
        auto binder_function = split_in_two(
            name, "Invalid line for binder specification! Must be: "
                  "name_of_type + <space or tab> + name_of_binder. Got: " +
                      line);
      ...
      if (bind) {
        auto binder_function = split_in_two(
            name, "Invalid line for add_on_binder specification! Must be: "
                  "name_of_type + <space or tab> + name_of_binder. Got: " +
                      line);
      ...
      if (bind) {
        auto binder_function = split_in_two(
            name, "Invalid line for binder_for_namespace specification! Must "
                  "be: name_of_type + <space or tab> + name_of_binder. Got: " +
                      line);
        binder_for_namespaces_[binder_function.first] =
            trim(binder_function.second);
      }
      ...
      if (bind) {
        auto binder_function = split_in_two(
            name,
            "Invalid line for add_on_binder_for_namespace specification! Must "
            "be: name_of_type + <space or tab> + name_of_binder. Got: " +
                line);
        add_on_binder_for_namespaces_[binder_function.first] =
            trim(binder_function.second);
      }
      ...
      holder_type_ = name_without_spaces;
      ...
      default_static_pointer_return_value_policy_ = name_without_spaces;
      ...
      default_member_pointer_return_value_policy_ = name_without_spaces;
      ...
      default_call_guard_ = name_without_spaces;

    else if (token == _prefix_for_static_member_functions_)
      prefix_for_static_member_functions_ = name_without_spaces;

    else if (token == _trampoline_member_function_binder_) {
      if (bind) {
        auto member_function_name_and_function_name = split_in_two(
            name,
            "Invalid line for trampoline_member_function_binder specification! "
            "Must be: qualified_class_name::member_funtion_name + <space or "
            "tab> + name_of_function. Got: " +
                line);
        custom_trampoline_functions_[member_function_name_and_function_name
                                         .first] =
            member_function_name_and_function_name.second;
      }
    }

    else {
      outs() << "Error: Invalid token in config file! Each token must be "
                "either: namespace, class or function! For example: '+function "
                "aaa::bb::my_function'. Token: '" +
                    token + "' Line: '" + line + '\''
             << "\n";
      assert(false);
      //			throw std::runtime_error("Invalid token in
      //config file! Each token must be either: namespace, class or function!
      //For example: '+function aaa::bb::my_function'. Token: '" + token +
      //									 "'
      //Line: '" + line + '\'');
    }
  }
...
bool Config::is_function_binding_requested(string const &function_) const {
  ...
  auto bind =
      std::find(functions_to_bind.begin(), functions_to_bind.end(), function);

  if (bind != functions_to_bind.end())
    return true;
...
bool Config::is_class_binding_requested(string const &class__) const {
  ...
  if (bind != classes_to_bind.end())
    return true;

  if (class_.rfind("std::shared_ptr<QuantLib::", 0) == 0)
    return true;
...
string
Config::is_custom_trampoline_function_requested(string const &function_) const {
  auto it = custom_trampoline_functions_.find(function_);
  if (it == custom_trampoline_functions_.end())
    return "";
  else
    return it->second;
}
...
bool Config::is_buffer_protocol_requested(string const &class__) const {
  ...
  auto buffer_protocol =
      std::find(buffer_protocols.begin(), buffer_protocols.end(), class_);
...
bool Config::is_module_local_requested(string const &namespace_) const {
  ...
  auto module_local_all =
      std::find(module_local_namespaces_to_add.begin(),
                module_local_namespaces_to_add.end(), namespace_all);
  if (module_local_all != module_local_namespaces_to_add.end()) {
    auto module_local_to_skip =
        std::find(module_local_namespaces_to_skip.begin(),
                  module_local_namespaces_to_skip.end(), namespace_);
  ...
  auto module_local_to_add =
      std::find(module_local_namespaces_to_add.begin(),
                module_local_namespaces_to_add.end(), namespace_);
  if (module_local_to_add != module_local_namespaces_to_add.end()) {
    ...
    if (module_local_to_skip != module_local_namespaces_to_skip.end()) {
      outs() << "Could not determent if namespace '" << namespace_
             << "' should use module_local or not... please resolve the "
                "conlficting options +module_local_namespace and "
                "-module_local_namespace!!!\n";
      assert(false);
      // throw std::runtime_error("Could not determent if namespace '" +
      // namespace_ + "' should use module_local or not... please resolve the
      // conlficting options +module_local_namespace and
      // -module_local_namespace!!!");
    }
    return true;
...
bool Config::is_smart_holder_requested(string const &class__) const {
  ...
  auto smart_held_class =
      std::find(smart_held_classes.begin(), smart_held_classes.end(), class_);
...
string Config::includes_code() const {
  std::ostringstream s;
  for (auto &i : includes_to_add)
    s << "#include " << i << "\n";
  if (O_include_pybind11_stl)
    s << "#include <pybind11/stl.h>\n";
