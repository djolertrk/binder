...
void add_relevant_includes(Context &context, clang::EnumDecl const *E,
  ...
  if (auto decl = E->getDefinition())
    add_relevant_include_for_decl(decl, includes);
}
...
bool is_bindable(EnumDecl const *E) {
  ...
  if (E->isCXXInstanceMember())
    return false;

  string name = E->getNameAsString(); // getQualifiedNameAsString(); //
  // if( name.rfind(')') != string::npos ) return false; // checking that this
  // is not an "(anonymous)" enum
  if (name.empty() or name.rfind(')') != string::npos)
    return false; // checking that this is not an "(anonymous)" enum
...
bool is_binding_requested(clang::EnumDecl const *E, Config const &config) {
  if (config.is_enum_binding_requested(E->getQualifiedNameAsString()))
    return true;

  bool bind =
      config.is_namespace_binding_requested(namespace_from_named_decl(E));
  // for( auto &t : get_type_dependencies(E) ) bind &= !is_skipping_requested(t,
  // config);
  return bind;
}
...
bool is_skipping_requested(clang::EnumDecl const *E, Config const &config) {
  ...
  if (config.is_enum_skipping_requested(qualified_name))
    return true;
  if (config.is_enum_binding_requested(qualified_name))
    return false;

  bool skip =
      config.is_namespace_skipping_requested(namespace_from_named_decl(E));

  // for( auto &t : get_type_dependencies(E) ) skip |= is_skipping_requested(t,
  // config);

  return skip;
}

// This function takes care about the LLVM/Clang bug which was fixed in
// LLVM6/Clang6. The body of the function is a backport from LLVM6.
std::string getQualifiedNameAsStringLLVM5Fix(NamedDecl const *E) {
  ...
  if ((E->getDeclName() || isa<DecompositionDecl>(E)))
    OS << *E;
  else
    OS << "(anonymous)";
  return correct;
...
std::string bind_enum(std::string const &module, EnumDecl const *E) {
  ...
  string r = "\tpybind11::enum_<{}>({}, \"{}\"{}, \"{}\"{})\n"_format(
      qualified_name, module, name, maybe_arithmetic,
      generate_documentation_string_for_declaration(E),
      module_local_annotation);

  // r += "\t // is_bindable " + E->getNameAsString() + " -> " +
  // std::to_string(is_bindable(E)) + "\n";

  for (auto e = E->enumerator_begin(); e != E->enumerator_end(); ++e) {
#if (LLVM_VERSION_MAJOR > 5)
    r += "\t\t.value(\"{}\", {})\n"_format(e->getNameAsString(),
                                           e->getQualifiedNameAsString());
#else
    r += "\t\t.value(\"{}\", {})\n"_format(
        e->getNameAsString(), getQualifiedNameAsStringLLVM5Fix(*e));
#endif
  }
  r.pop_back();
...
void EnumBinder::request_bindings_and_skipping(Config const &config) {
  if (is_skipping_requested(E, config))
    Binder::request_skipping();
  else if (is_binding_requested(E, config))
...
void EnumBinder::bind(Context &context) {
  ...
  string const module_variable_name =
      context.module_variable_name(namespace_from_named_decl(E));
