...
// Return the python operator that maps to the C++ operator; returns "" if no
// mapping exists This correctly handles operators that have multiple meanings
// depending on their argument count For example, operator_(this, other) maps to
// __sub__ while operator-(this) maps to __neg__
...
string function_arguments(clang::FunctionDecl const *record) {
  ...
  for (uint i = 0; i < record->getNumParams(); ++i) {
    // r +=
    // standard_name(record->getParamDecl(i)->getOriginalType().getCanonicalType().getAsString());
...
// n - number of arguments to generate. If n > num_of_function_parameters -
// generate only list with num_of_function_parameters
pair<string, string>
function_arguments_for_lambda(clang::FunctionDecl const *record, uint n) {
  string r, a;

  for (uint i = 0; i < record->getNumParams() and i < n; ++i) {
    QualType qt = record->getParamDecl(i)->getOriginalType();
    r += standard_name(qt) + ' ';
    if (!qt->isReferenceType() and !qt->isPointerType())
      r += !qt.isConstQualified() ? "const & " : "& ";
    r += "a" + std::to_string(i);
    a += "a" + std::to_string(i);
...
// Generate three version of function argument list: (with types separate by
// comma, only arguments names, only argument names with by-reference arguments
// converted to pointers by adding '&' example: ("string const & a0, int *a1,
// float a2", "a0, a1, a2", "&a0, a1, a2")
tuple<string, string, string>
function_arguments_for_py_overload(clang::FunctionDecl const *record) {
  string r, a, p;
  ...
  for (uint i = 0; i < record->getNumParams(); ++i) {
    ...
    a += "a" + std::to_string(i);
    p += string(qt->isLValueReferenceType() ? "&" : "") + "a" +
         std::to_string(i);
    if (i + 1 != record->getNumParams()) {
      r += ", ";
      a += ", ";
...
string template_specialization(FunctionDecl const *F) {
  ...
  if (F->getTemplatedKind() == FunctionDecl::TK_MemberSpecialization or
    ...
    // a->isDependent() << " :" << a->isInstantiationDependent() <<
    // "\n";
    // 	}
    // }
    ...
    if (TemplateArgumentList const *ta = F->getTemplateSpecializationArgs()) {
      ...
      for (uint i = 0; i < ta->size(); ++i) {
        ...
        // outs() << "function: '" << F->getNameAsString() << "' template
        // argument[" << i << "]=" << template_argument_to_string( ta->get(i) )
        // << " kind:" << ta->get(i).getKind() << "\n";

        // if( ta->get(i).isDependent() ) break;  // avoid explicitly specifying
        // SFINAE related arguments if( ta->get(i).getKind() ==
        // TemplateArgument::Expression ) break;  // avoid explicitly specifying
        // SFINAE related arguments

        if (ta->get(i).getKind() == TemplateArgument::ArgKind::Pack and
            !ta->get(i).pack_size())
          continue; // skipping `<>` at the end of parameter packs
        ...
        if (ta->get(i).getKind() == TemplateArgument::ArgKind::Pack and
          ...
          arg = arg.substr(
              1, arg.size() -
                     2); // removing extra <> around template parameter pack
        templ += arg + ",";
...
string python_function_name(FunctionDecl const *F) {
  if (F->isOverloadedOperator()) {
    ...
    // ) outs() << namespace_from_named_decl(F)
    // << "::" << F->getNameAsString() << "\n";
    // }

    // there is no point of generating different names for different template
    // function since in most cases we can treat them as overload's return
    // mangle_type_name( F->getNameAsString() + template_specialization(F) );
    return F->getNameAsString();
  }
}

// Generate function pointer type string for given function: void (*)(int,
// doule)_ or  void (ClassName::*)(int, doule)_ for memeber function
string function_pointer_type(FunctionDecl const *F) {
  ...
  if (auto m = dyn_cast<CXXMethodDecl>(F)) {
    prefix =
        m->isStatic()
            ? ""
            : class_qualified_name(cast<CXXRecordDecl>(F->getParent())) + "::";
...
// generate qualified function name that could be used in bindings code
// indcluding template specialization if any
string function_qualified_name(FunctionDecl const *F, bool omit_return_type) {
  string maybe_const;
  if (auto m = dyn_cast<CXXMethodDecl>(F))
    maybe_const = m->isConst() ? " const" : "";

  string r = (omit_return_type ? "" : standard_name(F->getReturnType()) + " ") +
             standard_name(F->getQualifiedNameAsString() +
...
// {
// 	return "C++: " + F->getQualifiedNameAsString() + "(" +
// function_arguments(F) + ')' + (F->isConst() ? " const" : "") + " --> " +
// standard_name( F->getReturnType().getCanonicalType().getAsString() )
// + '\n';
// }

// generate vector<QualType> with all types that function uses including: return
// type, types of function arguments and template arguments
...
bool is_binding_requested(FunctionDecl const *F, Config const &config) {
  if (config.is_function_binding_requested(F->getQualifiedNameAsString()) or
      config.is_function_binding_requested(function_qualified_name(F, true)))
    return true;
  ...
  for (auto &t : get_type_dependencies(F))
    bind |= binder::is_binding_requested(t, config);
...
bool is_skipping_requested(FunctionDecl const *F, Config const &config) {
  ...
  string qualified_name_with_args_info_and_template_specialization =
  ...
  bool skip =
      config.is_namespace_skipping_requested(namespace_from_named_decl(F));

  // moved to config -> qualified_name.erase(std::remove(name.begin(),
  // name.end(), ' '), name.end());
  skip |= config.is_function_skipping_requested(qualified_name);

  // calculating skipping for template classes without template specialization
  // specified as: myclass::member_function_to_skip outs() << "Checking skipping
  // for function: " << function_qualified_name(F, true) << "...\n";
  ...
  if (CXXMethodDecl const *M = dyn_cast<CXXMethodDecl>(F)) {
    ...
    if (dyn_cast<ClassTemplateSpecializationDecl>(C)) {
      ...
      // << "\n";
      skip |= config.is_function_skipping_requested(standard_name(
          C->getQualifiedNameAsString() + "::" + F->getNameAsString()));
    }
  }
  // outs() << "OK\n";
...
string bind_function(FunctionDecl const *F, uint args_to_bind,
  ...
  string function_qualified_name = standard_name(
      parent ? class_qualified_name(parent) + "::" + F->getNameAsString()
  ...
  if (m and m->isStatic()) {
    ...
    function_name =
        Config::get().prefix_for_static_member_functions() + function_name;
  ...
  if (args_to_bind == F->getNumParams() and (not always_use_lambda)) {
    function =
        "({}) &{}{}"_format(function_pointer_type(F), function_qualified_name,
                            template_specialization(F));
    ...
    if (documentation.size())
      documentation += "\\n\\n";
    documentation +=
        "C++: " + standard_name(F->getQualifiedNameAsString() + "(" +
                                function_arguments(F) + ')' +
                                (m and m->isConst() ? " const" : "") + " --> " +
                                standard_name(F->getReturnType()));
  } else {
    pair<string, string> args = function_arguments_for_lambda(F, args_to_bind);
    // string args; for(uint i=0; i<args_to_bind; ++i) args += "a" +
    // std::to_string(i) + ( i+1 == args_to_bind ? "" : ", " );

    string return_type = standard_name(F->getReturnType());

    // workaround of GCC bug during lambda specification: replace
    // enum/struct/class/const_* from begining of the lambda return type with
    // //const*
    ...
    if (m and !m->isStatic()) {
      // string object = class_qualified_name( m->getParent() ) + (m->isConst()
      // ? " const" : "") + " &o" + ( args_to_bind ? ", " : "" );
      string object =
          class_qualified_name(parent ? parent : m->getParent()) +
          (m->isConst() ? " const" : "") + " &o" +
          (args_to_bind
               ? ", "
               : ""); // forcing object type to be of parent class so member
                      // function with lifted access could be used
      function = "[]({}{}) -> {} {{ return o.{}({}); }}"_format(
          object, args.first, return_type, F->getNameAsString(), args.second);
    } else {
      function = "[]({}) -> {} {{ return {}({}); }}"_format(
          args.first, return_type, function_qualified_name, args.second);
    }
  }
  ...
  if (m and !m->isStatic()) {
    if (F->getReturnType()->isPointerType())
      maybe_return_policy =
          ", " + Config::get().default_member_pointer_return_value_policy();
    else if (F->getReturnType()->isLValueReferenceType())
      maybe_return_policy =
          ", " +
          Config::get().default_member_lvalue_reference_return_value_policy();
    else if (F->getReturnType()->isRValueReferenceType())
      maybe_return_policy =
          ", " +
          Config::get().default_member_rvalue_reference_return_value_policy();
  } else {
    if (F->getReturnType()->isPointerType())
      maybe_return_policy =
          ", " + Config::get().default_static_pointer_return_value_policy();
    else if (F->getReturnType()->isLValueReferenceType())
      maybe_return_policy =
          ", " +
          Config::get().default_static_lvalue_reference_return_value_policy();
    else if (F->getReturnType()->isRValueReferenceType())
      maybe_return_policy =
          ", " +
          Config::get().default_static_rvalue_reference_return_value_policy();
  }
  ...
  if (!Config::get().default_call_guard().empty())
    r += ", pybind11::call_guard<" + Config::get().default_call_guard() + ">()";
...
// Generate binding for given function. If function have default arguments
// generate set of bindings by creating separate bindings for each argument with
// default. if parent is not nullptr then bind function as-if it a member of
...
string bind_function(string const &module, FunctionDecl const *F,
  ...
  for (int i = 0; i < num_params; ++i) {
    // we want to avoid generating any lambda expression when any of the
    // function arguments have function-type
    if (is_function_type(
            F->getParamDecl(i)->getOriginalType().getCanonicalType())) {
  ...
  for (; args_to_bind < num_params; ++args_to_bind) {
    if (F->getParamDecl(args_to_bind)->hasDefaultArg())
      break;
  }
...
bool is_bindable_raw(FunctionDecl const *F, Context &context) {
  ...
  // if( F->getQualifiedNameAsString().rfind(')') != std::string::npos ) return
  // false;
  auto qualified_name = F->getQualifiedNameAsString();
  auto closing_bracket = qualified_name.rfind(')');
  if (closing_bracket != std::string::npos) {
    if (closing_bracket > 0 and qualified_name[closing_bracket - 1] == '(') {
    } // operator()
    else
      return false;
  }
  ...
  // bool r = true;
  bool r = !F->isDeleted(); //  and  !F->isVariadic(); disabled, instead we
                            //  force bindings with lambda for variadic

  if (F->isOverloadedOperator()) {
    bool isFriend = false;
    if (F->isFriend()) {
      // outs() << "Skipping a function marked as friend: " <<
      // F->getNameAsString() << '\n';
      isFriend = true;
    }
  ...
  r &= F->getTemplatedKind() !=
           FunctionDecl::TK_FunctionTemplate /*and  !F->isOverloadedOperator()*/
       and !isa<CXXConversionDecl>(F) and !F->isDeleted();
...
bool is_overloadable(CXXMethodDecl const *M) {
  // Pybind11 now allow returning a reference to primitive types or string so
  ...
  if( ReferenceType const *rt = dyn_cast<ReferenceType>( qt.getTypePtr() ) ) {
          ...
          static vector<string> const types_to_skip = {"std::string &", "const
  std::string &"}; for(auto const &t : types_to_skip) if( r == t) return false;
  }
...
void FunctionBinder::bind(Context &context) {
  ...
  string const module_variable_name =
      context.module_variable_name(namespace_from_named_decl(F));
  string const include = relevant_include(F);

  code() = "\t// " + F->getQualifiedNameAsString() + "(" +
           function_arguments(F) + ") file:" +
