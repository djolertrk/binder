...
public:
  ...
  virtual bool VisitFunctionDecl(FunctionDecl *F) {
    ...
    if (binder::is_bindable(F)) {
      ...
      context.add(b);
               ...
               F->getNameAsString() == "operator<<") {
      // outs() << "Adding insertion operator: " <<
      // binder::function_pointer_type(F) << "\n";
  ...
  // virtual bool
  // VisitClassTemplateSpecializationDecl(ClassTemplateSpecializationDecl *C) {
  // 	if( FullSourceLoc(C->getLocation(), ast_context->getSourceManager()
  // ).isInSystemHeader() ) return true; 	errs() << "Visit
  // ClassTemplateSpecializationDecl:" << C->getQualifiedNameAsString() <<
  // binder::template_specialization(C) << "\n"; 	C->dump();
  //     return true;
  // }
...
//      if (config.should_bind_typedef(T->getName().str()))
          auto* ns =  dyn_cast<NamespaceDecl>(T->getDeclContext());

          if (ns) {
            auto ns_name = ns->getQualifiedNameAsString();
            if (ns_name == "QuantLib") {
              llvm::outs() << "Namespace: " << ns_name << " Typedef: " << T->getName().str() << "\n";
  ...
  virtual bool VisitEnumDecl(EnumDecl *E) {
    if (E->isCXXInstanceMember() or E->isCXXClassMember())
      return true;

    binder::BinderOP b =
        std::make_shared<binder::EnumBinder>(E /*->getCanonicalDecl()*/);
    context.add(b);
...
public:
  ...
  explicit BinderASTConsumer(CompilerInstance *ci)
      : visitor(new BinderVisitor(ci)) {}
...
#else
  // CommonOptionsParser op(argc, argv, BinderToolCategory);
  if (llvm::Expected<CommonOptionsParser> eop =
          CommonOptionsParser::create(argc, argv, BinderToolCategory)) {
