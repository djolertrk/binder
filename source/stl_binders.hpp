...
public:
  vector_binder(pybind11::module &m, std::string const &name,
    ...
    // cl.def("resize", (void (Vector::*) (size_type count)) & Vector::resize,
    // "changes the number of elements stored");

    cl.def("empty", &Vector::empty, "checks whether the container is empty");
    cl.def("max_size", &Vector::max_size,
           "returns the maximum possible number of elements");
    cl.def("reserve", &Vector::reserve, "reserves storage");
    cl.def("capacity", &Vector::capacity,
           "returns the number of elements that can be held in currently "
           "allocated storage");
    cl.def("shrink_to_fit", &Vector::shrink_to_fit,
           "reduces memory usage by freeing unused memory");
    cl.def("clear", &Vector::clear, "clears the contents");
    cl.def("swap", (void(Vector::*)(Vector &)) & Vector::swap,
           "swaps the contents");
...
class map_binder {
public:
  map_binder(pybind11::module &m, std::string const &key_name,
             std::string const &value_name, std::string const &comparator_name,
    ...
    std::string maybe_extra =
        ((comparator_name == "std_less_" + key_name + "_t")
             ? ""
             : ("_" + comparator_name)) +
        ((allocator_name ==
          "std_allocator_std_pair_const_" + key_name + '_' + value_name + "_t")
             ? ""
             : ("_" + allocator_name));

    Class_ cl = pybind11::bind_map<Map, holder_type>(
        m, "map_" + key_name + '_' + value_name + maybe_extra);
  }
};
