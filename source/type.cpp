...
bool is_binding_requested(clang::QualType const &qt, Config const &config) {
  if (clang::PointerType const *pt =
    ...
    return is_binding_requested(pt->getPointeeType(), config);

  if (ReferenceType const *rt = dyn_cast<ReferenceType>(qt.getTypePtr()))
    return is_binding_requested(rt->getPointeeType(), config);

  if (CXXRecordDecl *r = qt->getAsCXXRecordDecl())
    return is_binding_requested(r, config);
...
bool is_skipping_requested(QualType const &qt, Config const &config) {
  if (clang::PointerType const *pt =
          dyn_cast<clang::PointerType>(qt.getTypePtr()))
    return is_skipping_requested(pt->getPointeeType(), config);

  if (ReferenceType const *rt = dyn_cast<ReferenceType>(qt.getTypePtr()))
    return is_skipping_requested(rt->getPointeeType(), config);

  if (CXXRecordDecl *r = qt->getAsCXXRecordDecl())
    return is_skipping_requested(r, config);

  return false;
}

// check if type some form of function-type (function type, function-pointer,
// function-reference, ...)
bool is_function_type(QualType const &qt) {
  if (dyn_cast<clang::FunctionType>(qt.getTypePtr()))
    return true;
  if (clang::PointerType const *pt =
          dyn_cast<clang::PointerType>(qt.getTypePtr()))
    return is_function_type(pt->getPointeeType());
  if (ReferenceType const *rt = dyn_cast<ReferenceType>(qt.getTypePtr()))
    return is_function_type(rt->getPointeeType());
  return false;
}

// extract include path needed for declaration itself (without template
// dependency if any), return empty string if no include could be found
string relevant_include(NamedDecl const *decl) {
  ...
  if (include.isValid()) {
    ...
    if (strlen(data) > 2 and
        (*data == '"' or *data == '<')) { // should be at least 3 chars:
                                          // open/close symbol + file name
      char terminator = *data == '"' ? '"' : '>';

      include_string.push_back(*data);
      ++data;
      for (; *data and *data != terminator; ++data)
        include_string.push_back(*data);
      if (*data == terminator)
        include_string.push_back(*data);
    }
    if (include_string.size() and include_string[0] == '"')
      include_string.resize(0); // avoid adding include in quotes because
...
// extract include path needed for declaration itself (without template
// dependency if any), do nothing if include could not be found (ie for
// build-in's)
void add_relevant_include_for_decl(
    NamedDecl const *decl,
    IncludeSet &includes /*, std::set<clang::NamedDecl const *> &stack*/) {
  static std::unordered_map<string, string> type_map;

  if (type_map.empty()) {
    std::vector<std::pair<string, std::vector<string>>> const
        include_types_map = {
            {"<algorithm>",
             {"std::move_backward", "std::iter_swap", "std::min"}},
            ...
            {"<functional>",
             {"std::function", "std::_Manager_operation",
            ...
            {"<locale>",
             {"std::ctype", "std::ctype_byname", "std::ctype_base",
              "std::locale", "std::money_base", "std::money_base::pattern",
              "std::messages_base", "std::numpunct", "std::num_get",
              "std::num_put", "std::numpunct_byname", "std::time_base",
              "std::codecvt", "std::codecvt_base", "std::codecvt_byname"}},
    ...
    for (auto const &include_types : include_types_map) {
      for (auto const &type : include_types.second)
        type_map.insert(make_pair(type, include_types.first));
    }
  }
  ...
  static vector<std::pair<string, string>> const name_map_old = {
      ...
      make_pair("std::__1::basic_string<char,std::__1::char_traits<char>,std::_"
                "_1::allocator<char>>",
                "<string>"),
      make_pair("std::__1::basic_streambuf<char,std::__1::char_traits<char>>",
                "<streambuf>"),
      make_pair("std::__1::basic_istream<char,std::__1::char_traits<char>>",
                "<istream>"),
      make_pair("std::__1::basic_iostream<char,std::__1::char_traits<char>>",
                "<ostream>"),
  };
  ...
  if (decl->isCXXClassMember()) {
    ...
    if (auto m = dyn_cast<CXXMethodDecl>(decl)) {
      ...
      // outs() << "CXXMethodDecl: looking for includes for type: " << name << "
      // (was: " << decl->getQualifiedNameAsString() <<") \n";
    }
    ...
    if (D->isCXXClassMember()) {
      if (auto c = dyn_cast<CXXRecordDecl>(D)) {
        ...
        // outs() << "CXXRecordDecl: looking for includes for type: " << name <<
        // " (was: " << D->getQualifiedNameAsString() <<") \n";
      }
    }
  } else {
  ...
  if (it != type_map.end()) {
    includes.add_include(O_annotate_includes ? it->second + " // " + name
                                             : it->second);
  ...
  for (auto &p : name_map_old) {
    if (begins_with(name, p.first)) {
      if (O_annotate_includes)
        includes.add_include(p.second + " // " + name);
      else
        includes.add_include(p.second);
      return;
    }
  }
...
bool is_bindable(QualType const &qt) {
  ...
  r &= !qt->isFunctionPointerType() and !qt->isRValueReferenceType() and
       !qt->isInstantiationDependentType() and
       !qt->isArrayType(); // and  !qt->isConstantArrayType()  and
                           // !qt->isIncompleteArrayType()  and
                           // !qt->isVariableArrayType()  and
                           // !qt->isDependentSizedArrayType()
  ...
  if (clang::PointerType const *pt =
    ...
    // if( pqt->isArithmeticType() ) return false;  // refuse to bind 'int*,
    // doublle*...' types
    ...
    if (begins_with(pqt_name, "struct std::pair") or
        begins_with(pqt_name, "const struct std::pair") or
        begins_with(pqt_name, "class std::tuple") or
  ...
  if (ReferenceType const *rt = dyn_cast<ReferenceType>(qt.getTypePtr())) {
    ...
    // special handling for std::pair&  and  std::tuple&  whitch pybind11 can't
    // pass by refference
    string pqt_name = standard_name(pqt.getAsString());
    if (begins_with(pqt_name, "struct std::pair") or
        begins_with(pqt_name, "class std::tuple"))
      return false; // but we allow bindings for 'const std::tuple' and 'const
                    // std::pair'

    // if( pqt_name == "char"  or  pqt_name == "wchar_t") return false; //
    // WARNING only TEMPORARY, until Pybind11 upstream is fixed

    // rt->dump();
    // outs() << "Ref " << qt.getAsString() << " -> " << is_bindable(
    // rt->getPointeeType().getCanonicalType() ) << "\n";
  ...
  if (Type const *tp = qt /*.getCanonicalType()*/.getTypePtrOrNull()) {
    if (CXXRecordDecl *rd = tp->getAsCXXRecordDecl()) {
      // outs() << "is_bindable qt CXXRecordDecl:" <<
      // rd->getQualifiedNameAsString() << " " << is_bindable(rd) << "\n";
      r &= is_bindable(rd);
    }
...
void request_bindings(clang::QualType const &qt, Context &context) {
  if (/*is_bindable(qt)  and*/ !is_skipping_requested(qt, Config::get())) {
    ...
    if (TagDecl *td = qt->getAsTagDecl()) {
      if (td->isCompleteDefinition() or
          dyn_cast<ClassTemplateSpecializationDecl>(td))
...
std::string standard_name(clang::QualType const &qt) {
  ...
  if (standard_names.count(r))
    return r;
  else
    return standard_name(qt.getCanonicalType().getAsString());
}
...
string standard_name_raw(string const &type) {
  static vector<std::pair<string, string>> const name_map = {
      make_pair("std::__1::",
                "std::"), // Mac libc++ put all STD objects into std::__1::  //
                          // WARNING: order is important here: we want to first
                          // replace std::__1:: so later we can change
                          // basic_string into string
      make_pair("std::__cxx11::", "std::"), // GCC libstdc++ 5.0+ puts all STD
                                            // objects into std::__cxx11::

      // make_pair("class std::", "std::"),
      // make_pair("struct std::", "std::"),

      make_pair("std::basic_string<char>", "std::string"),
      make_pair("std::basic_string<char, std::char_traits<char>, "
                "std::allocator<char> >",
                "std::string"),
      make_pair("class std::basic_string<char, struct std::char_traits<char>, "
  ...
  if (begins_with(r, "std::"))
    r = "std::" + simplify_std_class_name(r.substr(5));
...
/// Attempt to simplify std:: name by removing unneded template arguments.
/// Function assume that there is no 'std::' namespaces prefix at the beginning
...
string simplify_std_class_name_raw(string const &type) {
  static std::map<string, string> const std_typedef_map{
      ...
      {"basic_stringbuf<char,std::char_traits<char>,std::allocator<char>>",
       "stringbuf"},
      {"basic_stringbuf<wchar_t,std::wchar_traits<wchar_t>,std::allocator<"
       "wchar_t>>",
       "wstringbuf"},
  ...
  if (regex_map.empty()) {
    vector<std::pair<string, string>> regex_arg_map = {
        ...
        make_pair("set<(.*), *std::less<\\1 *>, *std::allocator<\\1 *> *>",
                  "set<\\1>"),
        make_pair("forward_list<(.*), *std::allocator<\\1 *> *>",
                  "forward_list<\\1>"),
        make_pair("map<(.*),(.*), *std::less<\\1 *>, "
                  "*std::allocator<std::pair<const \\1, \\2 *> *> *>",
                  "map<\\1,\\2>"),

        make_pair("multiset<(.*), *std::less<(.*)>, *std::allocator<(.*)>>",
                  "multiset<\\1 *>"),
        make_pair("multimap<(.*),(.*), *std::less<\\1>, "
                  "*std::allocator<std::pair<const \\1, \\2 *> *> *>",
                  "multimap<\\1,\\2>"),

        // LLVM-3.7
        // make_pair("^std::multimap<(.*),(.*),std::less<\\1>,std::allocator<std::pair<const
        // \\1, \\2> >>", "multimap<\\1,\\2>"),

        make_pair("unordered_set<(.*), "
                  "*std::hash<\\1>,std::equal_to<\\1>,std::allocator<\\1>>",
                  "unordered_set<\\1>"),
        make_pair("unordered_map<(.*), "
                  "*(.*),std::hash<\\1>,std::equal_to<\\1>,std::allocator<std::"
                  "pair<const \\1, \\2 *> *> *>",
                  "unordered_map<\\1,\\2>"),

        make_pair("unordered_multiset<(.*), *std::hash<\\1>, "
                  "*std::equal_to<\\1>, *std::allocator<\\1 *> *>",
                  "unordered_multiset<\\1>"),
        make_pair("unordered_multimap<(.*), *(.*),std::hash<\\1 *>, "
                  "*std::equal_to<\\1 *>, *std::allocator<std::pair<const \\1, "
                  "\\2 *> *> *>",
                  "unordered_multimap<\\1,\\2>"),
    };

    for (auto &p : regex_arg_map)
      regex_map.emplace_back(llvm::Regex(p.first), p.second);
  }
...
/// check if given class/struct is builtin in Python and therefor should not be
/// binded
bool is_python_builtin(NamedDecl const *C) {
  ...
  static std::set<string> const known_builtin = {
      ...
      "std::less", // lead to an error in include detection code, probably could
                   // be fixed
...
bool is_banned_symbol(clang::NamedDecl const *D) {
  static std::unordered_set<string> const known_banned_symbols = {
      ...
      // we need allocator bindings for some default constructors in std:: //
      ...
      // "std::__detail::_Node_iterator_base",
      // "std::__detail::_Node_const_iterator", "std::_Deque_iterator",
...
// 		 // we need allocator bindings for some default constructors in
// std:: // "std::allocator", "std::__allocator_destructor",
