...
std::vector<std::string> split(std::string const &buffer,
...
std::string replace_(std::string const &s, std::string const &from,
                     std::string const &to);
...
/// Try to read existing file. If file content does not match given code,
/// overwrite the file with code. Also create nested dirs starting from prefix
/// if nessesary.
...
std::string mangle_type_name(std::string const &name,
                             bool mark_template = true);

/// generate C++ comment line for given declaration along with file path and
/// line number
...
/// extract doc string (Doxygen comments) for given declaration and convert it
/// to C++ code
