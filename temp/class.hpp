...
std::string class_name(clang::CXXRecordDecl const *C, binder::Context &ctx, bool avoid_type_def);
...
// generate qualified class name that could be used in bindings code including
...
std::vector<clang::QualType> get_type_dependencies(
    clang::CXXRecordDecl const *C /*, bool include_members=false*/);

// generate vector<NamedDecl const *> with all declarations related to class
std::vector<clang::NamedDecl const *>
...
public:
  ...
  /// Generate string id that uniquely identify C++ binding object. For
  /// functions this is function prototype and for classes forward declaration.
...
private:
  ...
  // vector of classes in which current class depend to be binded (usually base
  // classes)
