...
namespace { // functions and constants that are only used in this translation
            // unit
...
string file_name_prefix_for_binder(BinderOP &b) {
  ...
  string exceptions = "_/<>.";
  include.erase(std::remove_if(include.begin(), include.end(),
                               [&](unsigned char c) {
                                 return not(std::isalnum(c) or
                                            std::find(exceptions.begin(),
                                                      exceptions.end(),
                                                      c) != exceptions.end());
                               }),
                include.end());
  ...
  if (include.size() <= 2) {
    ...
    // outs() << "Warning: file_name_prefix_for_binder could not determent file
    // name for decl: " + string(*b) + ", result is too short!\n";
  } // throw std::runtime_error( "file_name_for_decl failed!!! include name for
    // decl: " + string(*b) + " is too short!");
  include = include.substr(1, include.size() - 2);

  if (namespace_from_named_decl(decl) == "std" or
      begins_with(namespace_from_named_decl(decl), "std::"))
    include =
        "std/" + (begins_with(include, "bits/") ? include.substr(5) : include);
...
string generate_include_directives(IncludeSet const &include_set) {
  string r;
  for (auto &i : std::set<string>(include_set.includes().begin(),
                                  include_set.includes().end()))
    if (!Config::get().is_include_skipping_requested(i))
      r += "#include " + i + '\n';
...
const char *module_function_suffix =
...
Context::global_insertion_operator(clang::CXXRecordDecl const *C) {
  string op_pointer = "std::ostream & (*)(std::ostream &, const " +
                      (C->isStruct() ? string("struct ") : string("class ")) +
                      class_qualified_name(C) + " &)";

  // outs() << "Looking for operator: " << op_pointer << "\n";

  auto it = insertion_operators.find(op_pointer);
  if (it != insertion_operators.end())
    return it->second;

  if (auto t = dyn_cast<ClassTemplateSpecializationDecl>(
          C)) { // if operator<< is template it will have different form:
                // std::ostream & (*)(std::ostream &, const
                // A<type-parameter-0-0> &)
    op_pointer = "std::ostream & (*)(std::ostream &, const " +
                 standard_name(C->getNameAsString()) +
                 "<"; // Note: we do not use `getQualifiedNameAsString` because
                      // if argument is templated it string representation will
                      // not have namespaces
    for (uint i = 0; i < t->getTemplateArgs().size(); ++i)
      op_pointer += "type-parameter-0-" + std::to_string(i) + ", ";
    op_pointer.pop_back();
    op_pointer.back() = '>';
  ...
  while (repeat) {
    ...
    for (auto b = binders.begin(); b != binders.end(); ++b) {
      if (CXXRecordDecl const *C = dyn_cast<CXXRecordDecl const>(
              (*b)->named_decl())) { // right not only query dependency if we
                                     // dealing with class
        std::vector<CXXRecordDecl const *> const dependencies =
            (*b)->dependencies();
        for (auto &c : dependencies) {
          ...
          outs() << "Pushing forward binding for " << class_qualified_name(c)
                 << "...\n";
          auto e = find_if(b, binders.end(), [&c](BinderOP const &p) -> bool {
            return dyn_cast<CXXRecordDecl const>(p->named_decl()) == c;
          });
          if (e == binders.end()) {
            if (!repeat) {
              errs()
                  << "ERROR: Could not find binder for type: " +
                         class_qualified_name(c) +
                         "!\nUsually cause for this is that type was only "
                         "forward declared. Please check that Binder input "
                         "include file have full declaration of this class.\n";
              std::exit(1);

              // throw std::runtime_error( "ERROR: Could not find binder for
              // type: " + class_qualified_name(c) + "!");
            }
            // just declare forward declaration for now instead
            // outs() << "Could not find binder for type: " +
            // class_qualified_name(c) + "... will use forward declaration
...
void Context::generate(Config const &config) {
  ...
  string root_module_full_file_name =
      config.prefix + '/' + root_module_file_name;
  std::ofstream root_module_file_handle(root_module_full_file_name);
  if (root_module_file_handle.fail()) {
    outs() << "ERROR: Can not open file " + root_module_full_file_name +
                  " for writing...\n";
  ...
  for (uint i = 0; i < binders.size(); ++i) {
    ...
    if (O_flat) {
      ...
      file_name =
          np + (file_names[np] ? "_" + std::to_string(file_names[np]) : "");
    }
    ++file_names[np];
    ...
    std::map<string, std::vector<string>> const &namespace_includes =
        Config::get().namespace_includes();
    if (namespace_includes.count(namespace_)) {
      std::vector<string> const &n_includes = namespace_includes.at(namespace_);
      for (auto const &i : n_includes)
        includes.add_include(i);
    }

    std::map<string, string> const &binder_for_namespaces =
        Config::get().binder_for_namespaces();
    if (binder_for_namespaces.count(namespace_)) {
      if (namespace_entrance[namespace_] == 0) {
        code += "\n\t{}(M(\"{}\"));\n"_format(
            binder_for_namespaces.at(namespace_), namespace_);
      }
      skip = true;
    }

    std::map<string, string> const &add_on_binder_for_namespaces =
        Config::get().add_on_binder_for_namespaces();
    if (add_on_binder_for_namespaces.count(namespace_) and code.empty()) {
      if (namespace_entrance[namespace_] == 0)
        code += "\n\t{}(M(\"{}\"));\n"_format(
            add_on_binder_for_namespaces.at(namespace_), namespace_);
    }

    for (; code.size() < config.maximum_file_length and i < binders.size() and
           namespace_ == namespace_from_named_decl(binders[i]->named_decl());
         ++i) {
      ...
      if (O_trace) {
        ...
        includes.add_include(O_annotate_includes ? "<iostream> // --trace"
                                                 : "<iostream>");
      }
    ...
    string shared_declare =
        "PYBIND11_DECLARE_HOLDER_TYPE(T, " + holder_type + "<T>)";
    string shared_make_opaque =
        "PYBIND11_MAKE_OPAQUE(" + holder_type + "<void>)";

    string const pybind11_include =
        "#include <" + Config::get().pybind11_include_file() + ">";
    code = generate_include_directives(includes) +
           fmt::format(module_header, pybind11_include, config.includes_code(),
                       shared_declare, shared_make_opaque) +
           prefix_code + "void " + function_name + module_function_suffix +
           "\n{\n" + code + "}\n";

    if (O_single_file)
      root_module_file_handle << "// File: " << file_name << '\n'
                              << code << "\n\n";
    else
  ...
  for (const auto& typedefs : TypeDefTypes) {
    ...
    for (const auto& class_decl : CXXClassDeclToType) {
      if (class_decl.second == typedefs.first) {
        ...
        string class_qualified_name = class_decl.first->getQualifiedNameAsString();
//        llvm::outs() << "class_string_name: " << class_qualified_name << " typedef to " << typedef_name << "\n";

        string pybind_name = mangle_type_name(class_name(class_decl.first, *this, true));
        
        llvm::outs() << "class_string_name: " << pybind_name << " typedef to " << typedef_name << "\n";
      }
    }
  ...
  if (O_single_file) {
    root_module_file_handle << "\n// Source list file: "
                            << root_module_prefix + ".sources\n";
    for (auto &s : sources)
      root_module_file_handle << "// " << s << "\n";

    root_module_file_handle << "\n// Modules list file: "
                            << root_module_prefix + ".modules\n// ";
