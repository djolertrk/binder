...
/// Try to read exisitng file and if content does not match to code - write a
/// new version. Also create nested dirs starting from prefix if nessesary.
void update_source_file(std::string const &prefix, std::string const &file_name,
  ...
  if (system(command_line.c_str()) != 0)
    throw std::runtime_error("ERROR: Command \"" + command_line + "\" failed");

  string full_file_name = prefix + '/' + file_name;
  std::ifstream f(full_file_name);
  std::string old_code((std::istreambuf_iterator<char>(f)),
                       std::istreambuf_iterator<char>());

  if (old_code != code) {
    if (O_verbose)
      outs() << "Writing " << full_file_name << "\n";
    std::ofstream f(full_file_name);
    if (f.fail()) {
      outs() << "ERROR: Can not open file " << full_file_name
             << " for writing...\n";
      assert(false);
      //	throw std::runtime_error("ERROR: Can not open file " +
      //full_file_name + " for writing...");
    }
    f << code;
  } else {
    if (O_verbose)
      outs() << "File " << full_file_name << " is up-to-date, skipping...\n";
  }
}
...
string namespace_from_named_decl(NamedDecl const *decl) {
  ...
  string path =
      qn.substr(0, namespace_len > 1 ? namespace_len - 2
                                     : namespace_len); // removing trailing '::'

  return path;
}

/// generate unique string representation of type represented by given
/// declaration
string typename_from_type_decl(TypeDecl const *decl) {
  return standard_name(
      decl->getTypeForDecl()
          ->getCanonicalTypeInternal() /*getCanonicalType()*/.getAsString());
}

/// Calculate base (upper) namespace for given one: core::pose::motif -->
/// core::pose
...
void fix_boolean_types(string &type) {
  ...
  while ((i = type.find(B, i)) != string::npos) {
    if ((i == 0 or
         (!std::isalpha(type[i - 1]) and !std::isdigit(type[i - 1]))) and
        (i + B.size() == type.size() or (!std::isalpha(type[i + B.size()]) and
                                         !std::isdigit(type[i + B.size()]))))
...
string template_argument_to_string(clang::TemplateArgument const &t) {
  ...
  // // simplify result if argument is a small number, taking care of cases like
  // 1L or 1UL if( r.size() < 5 and std::isdigit(r[0]) ) { 	if( ends_with(r, "UL"
  // ) ) r.resize( r.size() - 2 ); 	else if( ends_with(r, "L" ) ) r.resize(
  // r.size() - 1 );
  // }
...
// generate C++ comment line for given declartion along with file path and line
// number: // core::scoring::vdwaals::VDWAtom
// file:core/scoring/vdwaals/VDWTrie.hh line:43
string generate_comment_for_declaration(clang::NamedDecl const *decl) {
  string const include = relevant_include(decl);
  return "// " + standard_name(decl->getQualifiedNameAsString()) + " file:" +
         (include.size() ? include.substr(1, include.size() - 2) : "") +
         " line:" + line_number(decl) + "\n";
}

// extract text from hierarchy of comments
string get_text(comments::Comment const *C, SourceManager const &SM,
                SourceLocation previous) {
  if (auto tc = dyn_cast<comments::TextComment>(C))
    return string(tc->getText());
  else {
    string r;

    if (isa<comments::ParagraphComment>(C))
      r += '\n';

    for (auto i = C->child_begin(); i != C->child_end(); ++i) {
#if (LLVM_VERSION_MAJOR < 8)
      if (SM.getSpellingLineNumber(previous) !=
          SM.getSpellingLineNumber((*i)->getLocStart())) { // getBeginLoc
...
#if (LLVM_VERSION_MAJOR >= 8)
      if (SM.getSpellingLineNumber(previous) !=
          SM.getSpellingLineNumber((*i)->getBeginLoc())) { // getBeginLoc
...
// extract doc string (Doxygen comments) for given declaration and convert it to
// C++ source code string
std::string
generate_documentation_string_for_declaration(clang::Decl const *decl) {
  string text;
